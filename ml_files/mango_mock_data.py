# ============================================================================
# MANGO CROP RECOMMENDATION MODEL - RANDOM FOREST WITH ONNX CONVERSION
...
print("=" * 70)
print("STEP 3: Generating mock training data for 5 crops...")
print("=" * 70)

np.random.seed(42)

# Define optimal soil parameters for each crop
# User-provided Mango data: N= 12 mg/kg, P = 7 mg/kg, K = 9 mg/kg, pH = 6.50, Temperature = 29.4°C, Moisture = 62%
crop_data = {
    'Banana': {
        'N': (100, 150),
        'P': (25, 50),
    ...
    'Mango': {
        'N': (10, 14),           # mg/kg - centered around user's 12
        'P': (5, 9),             # mg/kg - centered around user's 7
        'K': (7, 11),            # mg/kg - centered around user's 9
        'pH': (6.2, 6.8),        # centered around user's 6.50
        'Moisture': (58, 66),    # % - centered around user's 62%
        'Temperature': (28, 31)  # °C - centered around user's 29.4°C
    }
}
...
axes[0, 0].set_title('Confusion Matrix - 5 Crops', fontsize=12, fontweight='bold')
...
test_samples = {
    ...
    'Mango': np.array([[12, 7, 9, 6.5, 62, 29.4]], dtype=np.float32)
}
...
for crop_name, sample_data in test_samples.items():
    ...
    print(f"  N: {sample_data[0,0]:.1f} mg/kg, P: {sample_data[0,1]:.1f} mg/kg, K: {sample_data[0,2]:.1f} mg/kg")
    print(f"  pH: {sample_data[0,3]:.2f}, Moisture: {sample_data[0,4]:.1f}%, Temperature: {sample_data[0,5]:.1f}°C")
...
print(f"\n🥭 USER-PROVIDED MANGO DATA:")
print(f"  N (Nitrogen): 12 mg/kg")
print(f"  P (Phosphorus): 7 mg/kg")
print(f"  K (Potassium): 9 mg/kg")
print(f"  pH Level: 6.50")
print(f"  Temperature: 29.4°C")
print(f"  Moisture: 62%")
